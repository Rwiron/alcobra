...
const validateServiceData = (data: ServiceRequestData) => {
    ...
    return {
        ...
        priceType: (data.priceType as 'fixed' | 'variable' | 'consultation') || 'fixed',
        serviceType: (data.serviceType as 'individual' | 'package' | 'addon') || 'individual',
        difficultyLevel: (data.difficultyLevel as 'basic' | 'intermediate' | 'advanced' | 'expert') || 'basic',
        categoryId,
        prerequisites: Array.isArray(data.prerequisites) ? data.prerequisites : [],
        tags: Array.isArray(data.tags) ? data.tags : [],
        preparationTime: data.preparationTime ? parseInt(data.preparationTime.toString()) : 0,
        cleanupTime: data.cleanupTime ? parseInt(data.cleanupTime.toString()) : 0,
